...
export const createUserSalary = async (req: Request, res: Response) => {
      ...
      try {
          ...
          if (!basicPay || !totalHours || !workingHours || !publicLeaveWorkingHour || 
              !overTimeHours || !overTimePayThisMonth || !publicHoursPayThisMonth || !totalSalaryThisMonth) {
            return APIResponse.error(res,'All fields are required' , StatusCodes.BAD_REQUEST);
          }
      
          // Validate numeric fields
          const numericFields = [basicPay, totalHours, workingHours,publicLeaveWorkingHour, 
                                 overTimeHours, overTimePayThisMonth, publicHoursPayThisMonth, totalSalaryThisMonth];
          
          if (numericFields.some(field => typeof field !== 'number' || isNaN(field))) {
            return APIResponse.error(res,'All fields must be valid numbers',StatusCodes.BAD_REQUEST);
          }
                    ...
                    if (existingSalary) {
                      return APIResponse.error(res, 'Duplicate y data found', StatusCodes.CONFLICT);
                    }
          ...
          return APIResponse.success(res,'User salary created successfully', savedUserSalary,StatusCodes.CREATED);
        } catch (error) {
          console.error('Error creating user salary:', error);
          return APIResponse.error(res,'An error occurred while creating user salary',StatusCodes.INTERNAL_SERVER_ERROR);
        }
...
export const getAllUsersSalary = async (req: Request, res: Response) => {
      ...
      //     if (!req.user || !req.user.hasPermission('VIEW_ALL_SALARIES')) {
      //       return APIResponse.error(res, 'Unauthorized access', StatusCodes.FORBIDDEN);
      //     }
          ...
          if (page < 1 || limit < 1) {
            return APIResponse.error(res, 'Invalid pagination parameters', StatusCodes.BAD_REQUEST);
          }
          ...
          if (!salaries || salaries.length === 0) {
            return APIResponse.success(res, 'No salary records found', [], StatusCodes.OK);
          }
...
export const getsingleUserSalary = async (req: Request, res: Response) => {
  try {
    ...
    if (!salary) {
      return APIResponse.error(res, 'Salary record not found for this user', StatusCodes.NOT_FOUND);
    }
...
export const updateUserSalary = async (req: Request, res: Response) => {
      ...
      //   console.log(req.params.id)
        console.log(userId)
        const { basicPay, totalHours, workingHours, publicLeaveWorkingHour, overTimeHours, overTimePayThisMonth, publicHoursPayThisMonth, totalSalaryThisMonth } = req.body;
